...
def param_table(fwhm, delta_fwhm, E_res, delta_E_res, h_max, f):
    num_el = len(fwhm)
    table = np.zeros((num_el,6))
    col_names = ['Scan no.', 'FWHM (eV)', 'ΔFWHM (meV)', 'E_res (eV)', 'ΔE_res (meV)', 'h_peak (a.u.)']
    for i in range(num_el):
        table[i,:] = [i+1, fwhm[i], delta_fwhm[i]*1000, E_res[i], delta_E_res[i]*1000, h_max[i]]
    #print(tabulate(table, headers='firstrow', tablefmt='fancy_grid', floatfmt=".4f"))
    table_df1 = pd.DataFrame(table, columns = col_names)
    table_df = table_df1.copy()
    table_df.update(table_df.applymap('{:,.4f}'.format))
    table_df['Scan no.'] = table_df['Scan no.'].astype(float).astype(int)
    ...
    ax.axis('off')
    ax.axis('tight')
    table_fig = ax.table(cellText = table_df.convert_dtypes().to_numpy(), colLabels = table_df.columns, 
                         colWidths = [0.1, 0.18, 0.18, 0.18, 0.18, 0.18], loc='center', cellLoc='center')
    table_fig.scale(1, 1.5)
    table_fig.auto_set_font_size(False)
    table_fig.set_fontsize(8)
    fig.tight_layout()
    plt.savefig(f.replace('.csv','.png').replace('/csv/','/Figures/'), dpi = 300)
    plt.close(fig)
    table_df1.to_csv(f, index=False, header=False)
...
def scan10_table(delta_fwhm, delta_E_res, f):
    table = [delta_fwhm[-1]*1000, delta_E_res[-1]*1000]
    #print(tabulate(table, headers='firstrow', tablefmt='fancy_grid', floatfmt=".4f"))
    table_df1 = pd.DataFrame([table], columns = ['ΔFWHM_cube (meV)', 'ΔE_res_cube (meV)'])
    table_df = table_df1.copy()
    table_df.update(table_df.applymap('{:,.4f}'.format))
    plt.rcParams["figure.figsize"] = [10, 7]
    plt.rcParams["figure.autolayout"] = True
    ...
    ax.axis('off')
    ax.axis('tight')
    table_fig = ax.table(cellText = table_df.values, colLabels = table_df.columns, loc='center', cellLoc='center')
    table_fig.scale(1, 2)
    table_fig.set_fontsize(16)
    fig.tight_layout()
    plt.savefig(f.replace('.csv','.png').replace('/csv/','/Figures/'), dpi = 300)
    plt.close(fig)
    table_df1.to_csv(f, index=False, header=False)
...
def scan1_table(fwhm, E_res, f):
    table = [fwhm[0], E_res[0]]
    #print(tabulate(table, headers='firstrow', tablefmt='fancy_grid', floatfmt=".4f"))
    table_df1 = pd.DataFrame([table], columns = ['FWHM (eV)', 'E_res (eV)'])
    table_df = table_df1.copy()
    table_df.update(table_df.applymap('{:,.4f}'.format))
    plt.rcParams["figure.figsize"] = [10, 7]
    plt.rcParams["figure.autolayout"] = True
...
# ## Function that generates a PDF report of all the plots and tables obtained for the given scan

# In[20]:


def get_report(cwd, fig_dir, csv_dir, export_list, f, dpi = 300):
    num_el = len(export_list)
    png_index = []
    csv_index = []
    for i in range(num_el):
        if export_list[i][-4:None] == '.png':
            png_index.append(i)
        elif export_list[i][-4:None] == '.csv':
            csv_index.append(i)
        else:
            continue
    images = []
    wanted_dpi = dpi
    for i in png_index:
        images.append(Image.open(cwd + fig_dir + export_list[i]))
    for i in csv_index:
        images.append(Image.open(cwd + fig_dir + export_list[i].replace('.csv','.png')))
    image_rgb = []
    for img in images:
        try:
            ...
            dpi_x, dpi_y = dpi, dpi
        dpi_x_resize = round(img.width*wanted_dpi/dpi_x)
        dpi_y_resize = round(img.height*wanted_dpi/dpi_y)
        img = img.resize((dpi_x_resize, dpi_y_resize), resample=Image.LANCZOS)
        img_rgb = Image.new('RGB', img.size, (255, 255, 255))
        img_rgb.paste(img, mask = img.split()[3])
        image_rgb.append(img_rgb)
    pdf_path = cwd + '/' + f
    image_rgb[0].save(pdf_path, "PDF" ,resolution=100.0, optimize=True, save_all=True, append_images=image_rgb[1:])
...
def Nanocube_Lorentzian(mode, f1, f2, title_list, export_list):
    ...
    if mode == 1:
        ...
        get_report(cwd, fig_dir, csv_dir, export_list, 'Single_scan_report.pdf')
    else:
        ...
        get_report(cwd, fig_dir, csv_dir, export_list, 'Consecutive_scan_report.pdf')
